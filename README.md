...
Охуенная картинка про модель оси

http://infocisco.ru/articles/encapsulation_pdu_osi.png

#### Расскажи про структуру сетевого пакета. Про структуру фрейма.



#### Какие бывают рейды? В чём разница?

Рейд1 - зеркало
рейд - страйп
Есть куча вариаций с выделенными или нет дисками "чётности".   
Разница в различной избыточности хранения данных, скорости ребилда, скорости доступа и записи данных. 
Не стоит забывать про программные и аппаратные.


#### как бы ты замерял производительность используемого рейда?

А производительность чего именно мы измеряем? Файловой системы или самого устройства? 
Под разные задачи разные утилиты.


#### В чём отличие коммутатора от маршрутизатора

концентратор - это тупой хаб, работающий широковещательно.
коммутатор работает на канальном уровне, т.е на более низком 
маршрутизатор на сетевом - более высоком. 
И деление это условно, вариантов оборудования дохуя.

#### Что такое транк? Что такое влан? Как работает маршутизатор? Как выглядит таблица маршрутизации? Как работает LACP?

В терминологии Циско - это тагированый порт на коммутаторе
Тагированый порт - это тот, в котором фреймы имеют таг, показывающий коммутатору, к какому влану относится фрейм. 

в общей терминологии, транком может называться сетевое соединение (физическое) между двумя устройствами
Ещё транком могут называть ситуацию, когда между двумя сетевыми устройствами более одного канала связи в параллель.

Влан - в двух словах не скажешь. Очень годная статья - читать её. 
http://xgu.ru/wiki/VLAN

LACP - протокол, предназначенный для объединения нескольких физических каналов в один логический в сетях Ethernet. Агрегированные каналы LACP используются как для повышения пропускной способности, так и повышения отказоустойчивости. 


#### Что ты сделал для себя в последнее время? Какой софт поставил по фану и потыкал?  

Ээээээ. Начал делать бороду на питоне! Поставил вмвар!
